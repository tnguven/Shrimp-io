import { expect, test as it, describe, vi, afterEach, beforeEach } from 'vitest';
import { ObjectId } from 'mongodb';
import { makeShortUrl as makeShortUrlInAction } from '.';
import { buildMakeShortUrl, getAddedDate, ShortUrl } from './short-url';

const mockGetShortUrl = vi.fn();
const mockIsValidId = vi.fn();
  ...
  describe('happy path', () => {
    ...
    describe('initialize from existing shortUrl', () => {
      ...
      it('expireAt should be update to default 7 days', () => {
        vi.useFakeTimers().setSystemTime(new Date('2022-03-01T00:00:00').getTime());
        const shortUrl = makeShortUrl({ ...defaultParams, ...existingShortUrl });
        expect(shortUrl.getCreatedAt().toISOString()).toEqual('2022-03-01T00:00:00.000Z');
